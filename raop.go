			...
			if len(a) > 1 {
				...
				if name == "timing_port" {
					...
					serverPort, controlPort, timePort := c.setup(port, func(b []byte, size int) {
						s.delegate.ReceivedAudioPacket(c, b)
					})
...
//process the RECORD requests (start or restart the audio streaming)
...
//process the FLUSH requests (pause the audio streaming)
...
//process the TEARDOWN requests (completely stop the audio streaming)
